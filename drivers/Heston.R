...
source('drivers/getImpliedVolatility.R')
source('models/bivariate/ModelHeston.R')

require(Rcpp)

sourceCpp('src/HestonFourierCosine.cpp')
...
args$nloptr<-list(maxiter=100,
                  ...
                  #rho, kappa, theta, sigma
                  l = c(-1.0 + eps,eps,eps, eps),
                  u = c(1.0-eps,4.0-eps,1.0-eps,2.0-eps),
...
res <- summary(output$solution,ModelHeston,x,delta,args)
