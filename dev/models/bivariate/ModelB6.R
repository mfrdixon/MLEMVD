...
# Date:   2/22/2017
# Notes:  Ported to R from the Matlab library 'ClosedFormMLE'
...
ModelB6<-function(x,x0,del,param){
  ...
  # 
  # The attached files contain the expressions for lnpX as a function of the coefficients c of order -1,0,1,2, i.e., order K<-2 in delta with my notation. This should be sufficient for most if not all practical applications. 
  # I'm assuming you'll use something like daily data or maybe weekly?
  # 
  # lnpX in the code is the log of the transition density of (x1,x2) given (x10,x20), separated by a time interval del. 
  # 
  # Because of the presence of the sqrt terms, you need to be careful not only to stay on the positive axis (using Feller's condition), but also to keep x2 and x20 well clear of zero. 
  # That is, you want the long term mean of x2, namely the parameter a/b to be sufficiently large. Otherwise, you get into boundary issues and the expansion needs an explicit boundary condition. 
  # If you calibrate this model to financial data, this should not be a problem. 
  # 
  # 
  # 
  ...
  s <- param[4]
  r <- param[5]
  # notes that the log(r) term has been changed to log(|r|) to returning -inf 
  Dv <- 0.5*log(r^2) + log(s) + log(x2)
