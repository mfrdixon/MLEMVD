...
#' @param param0 The maximum likelihood estimated parameter
#' @param args Passed to internal functions
...
summary<-function(logdensity,x,del,param0,args=NULL){
  ...
  info <- list()
  opt<- objfun(param0)
  info$score <- pracma::grad(objfun,param0)
  info$H <- pracma::hessian(objfun,param0)
  ...
  if (args$plot==TRUE){
    ...
    par(mfrow=c(pracma::ceil(n.param/2),2))
