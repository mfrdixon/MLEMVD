getChain<-function(fileName){
  ...
  ZNGA_f<-read.csv(fileName)
  
  keys<-unique(ZNGA_f$TIMESTAMP)
  n<- dim(ZNGA_f)[1]
  
  setClass("Option", representation(symbol = "character", timestamp="character", underlying = "numeric", strike = "numeric", mid_price = "numeric", maturity = "numeric", type="character"))
